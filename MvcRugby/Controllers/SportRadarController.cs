...
namespace MvcRugby.Controllers
{
    ...
    {       
        ...
        public async Task<IActionResult> RoundLineup(string? id) //, int? roundNr
        {
            // Get SportsRadar data from Api via services, using id passed from Index() view by the user selection
            ...
            if (roundLineup != null && roundLineup.lineups != null)
            {
                ...
                {
                    if (sportEvent?.sport_event?.sport_event_context?.round?.number == 4) //roundNr
                    {
                        ...
                        {
                            // if there are teams, then for each team in the round, 
                            foreach (var competitor in sportEvent.lineups.competitors)
                            {
                                // Add the team info to the teamviewModel
                                ...
                                ViewModel.Teams.Add(TeamViewModel);
                            }
                        }              
                        return View(ViewModel);
                    }
